...
class DeliverySkipped(Exception):
    """Nothing to send to (no active config, blank target, or the backend package is not installed)."""


def deliver(delivery: Delivery) -> Delivery:
    """Send a pending delivery once; ends `sent`, `skipped` (with a warning) or `failed`."""
    ...
    try:
        ...
        logger.warning("Notification %s: %s delivery failed: %s", notification.pk, delivery.kind, error)
        return _finish(delivery, DeliveryStatus.FAILED, error)
    return _finish(delivery, DeliveryStatus.SENT)
...
def _finish(delivery: Delivery, status: str, error: str = "") -> Delivery:
    ...
    delivery.save(update_fields=["status", "attempts", "last_error", "sent_at", "modified_at"])
    return delivery
